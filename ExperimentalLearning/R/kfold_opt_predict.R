...
#' @title *Construct an ensemble model using the entire data set (following a computationally optimised procedure)  and generate CV-based (out of sample predictions) based on this model.
#'
#' @description *.
#' 
#' @export
...
kfold_opt_predict <- function(df, LHS, RHS, RHS_ext, permute, perm_test, control_var, control_var_unbalanced,
  ...
  # OOS predictions
  # ---------------------------------
  oospredictreturn <- oos_predict(df=df_temp[get(holdout_var)==FALSE],LHS=LHS,RHS=RHS,cluster_var=cluster_var,
                          from=1:nrow(df_temp[get(holdout_var)==FALSE]),
                          to=1:nrow(df_temp[get(holdout_var)==FALSE]),
