...
## NOTE: Code source: https://github.com/jeffwong/glmnet/blob/master/R/glmnet.R (modification:
## disable control that causes an error when providing an x matrix with only one column, i.e. 
## in the single predictor case)

glmnet_custom <- function(x,y,family=c("gaussian","binomial","poisson","multinomial",
  "cox","mgaussian"),weights,offset=NULL,alpha=1.0,nlambda=100,
  ...
  exclude,penalty.factor=rep(1,nvars),lower.limits=-Inf,upper.limits=Inf,
  maxit=100000,type.gaussian=ifelse(nvars<500,"covariance","naive"),
